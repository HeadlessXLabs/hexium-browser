"""Stealth detection tests for hexium_browser.
...
PROXY = os.environ.get("HEXIUM_TEST_PROXY")


@pytest.fixture(scope="module")
def browser():
    """Shared browser instance for stealth tests."""
    b = launch(headless=True, humanize=True, proxy=PROXY)
    yield b
    b.close()
...
class TestIssueRegressions:
    ...
    def test_add_init_script_with_proxy(self, browser):
        ...
        proxy = os.environ.get("HEXIUM_TEST_PROXY")
        if not proxy:
            pytest.skip("HEXIUM_TEST_PROXY not set")
