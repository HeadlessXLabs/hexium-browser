...
class TestSyncIsolatedWorld:
    ...
    def _make_world(self, cdp_send_side_effect=None):
        ...
        from hexium_browser.human import _SyncIsolatedWorld
...
class TestAsyncIsolatedWorld:
    ...
    def _make_world(self, cdp_send_side_effect=None):
        from hexium_browser.human import _AsyncIsolatedWorld
...
class TestIsInputElementStealth:
    ...
    def test_uses_isolated_world_when_available(self):
        from hexium_browser.human import _is_input_element
...
class TestAsyncIsInputElementStealth:
    ...
    async def test_uses_isolated_world_when_available(self):
        from hexium_browser.human import _async_is_input_element
...
class TestIsSelectorFocusedStealth:
    ...
    def test_uses_isolated_world_when_available(self):
        from hexium_browser.human import _is_selector_focused
...
class TestAsyncIsSelectorFocusedStealth:
    ...
    @pytest.mark.asyncio
    async def test_uses_isolated_world_when_available(self):
        from hexium_browser.human import _async_is_selector_focused
...
class TestShiftSymbolCDPSync:
    ...
    def test_shift_symbol_codes_completeness(self):
        ...
        from hexium_browser.human.keyboard import SHIFT_SYMBOLS, _SHIFT_SYMBOL_CODES, _SHIFT_SYMBOL_KEYCODES
    ...
    def test_cdp_path_sends_key_down_and_key_up(self):
        ...
        from hexium_browser.human.keyboard import _type_shift_symbol
        from hexium_browser.human.keyboard import _SHIFT_SYMBOL_CODES, _SHIFT_SYMBOL_KEYCODES
    ...
    def test_cdp_path_all_shift_symbols(self):
        ...
        from hexium_browser.human.keyboard import _type_shift_symbol, SHIFT_SYMBOLS
...
class TestShiftSymbolCDPAsync:
    ...
    async def test_cdp_path_sends_events_async(self):
        from hexium_browser.human.keyboard_async import _type_shift_symbol
        from hexium_browser.human.keyboard import _SHIFT_SYMBOL_CODES, _SHIFT_SYMBOL_KEYCODES
...
class TestNavigationInvalidation:
    ...
    def test_goto_invalidates_stealth_world_sync(self):
        from hexium_browser.human import patch_page, _CursorState
        from hexium_browser.human.config import resolve_config
...
class TestPatchPageStealthWiring:
    ...
    def test_patch_page_sets_stealth_world(self):
        from hexium_browser.human import patch_page, _CursorState, _SyncIsolatedWorld
    ...
    def test_patched_evaluate_uses_isolated_world(self):
        """page.evaluate must not go through Playwright UtilityScript (sourceUrlLeak)."""
        from hexium_browser.human import patch_page, _CursorState

        page, cdp = self._make_mock_page()
        orig_evaluate = MagicMock(return_value="PW")
        page.evaluate = orig_evaluate
        ...
        def _send(method, params=None):
            ...
            if method == "Runtime.evaluate":
                return {"result": {"value": {"failing": [], "totalFails": 0}}}
            return {}

        cdp.send = MagicMock(side_effect=_send)
        patch_page(page, _cfg(), _CursorState())
        out = page.evaluate("() => ({ failing: [], totalFails: 0 })")
        assert out == {"failing": [], "totalFails": 0}
        orig_evaluate.assert_not_called()
        eval_params = next(
            c.args[1] for c in cdp.send.call_args_list if c.args[0] == "Runtime.evaluate"
        )
        assert eval_params["contextId"] == 7
        assert eval_params.get("awaitPromise") is True
        assert "UtilityScript" not in eval_params["expression"]

    def test_ensure_stealth_evaluate_is_idempotent(self):
        from hexium_browser.human import ensure_stealth_evaluate, patch_page, _CursorState

        page, cdp = self._make_mock_page()
        patch_page(page, _cfg(), _CursorState())
        wrapped = page.evaluate
        ensure_stealth_evaluate(page, is_async=False)
        assert page.evaluate is wrapped
    ...
    def test_stealth_world_none_when_cdp_fails(self):
        """CDP failure at patch time still leaves a lazy isolated world for evaluate."""
        from hexium_browser.human import patch_page, _CursorState, _SyncIsolatedWorld
    ...
    def test_click_passes_through_stealth_dom_query(self):
        ...
        with mock_patch(
            "hexium_browser.human.scroll_to_element",
            return_value=(fake_box, 200.0, 215.0, False),
        ), mock_patch(
            "hexium_browser.human.ensure_actionable",
        ), mock_patch(
            "hexium_browser.human.check_pointer_events",
        ):
            try:
                page.click("#btn")
            except Exception:
...
class TestShiftSymbolMaps:
    ...
    def test_all_codes_are_valid_key_codes(self):
        from hexium_browser.human.keyboard import _SHIFT_SYMBOL_CODES
    ...
    def test_all_keycodes_are_positive_integers(self):
        from hexium_browser.human.keyboard import _SHIFT_SYMBOL_KEYCODES
    ...
    def test_shift_symbols_set_matches_codes_keys(self):
        from hexium_browser.human.keyboard import SHIFT_SYMBOLS, _SHIFT_SYMBOL_CODES
...
def _launch_kwargs(**extra):
    ...
    kw = {"humanize": True, "headless": False, "show_cursor": True}
    kw.update(extra)
    if not kw.get("proxy"):
        kw.pop("proxy", None)
    return kw
