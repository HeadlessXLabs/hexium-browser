"""Human-like behavioral layer for hexium_browser.
...
logger = logging.getLogger("hexium_browser.human")
...
_EVAL_ARG_UNSET = object()


def _expression_is_function(expression: str) -> bool:
    s = expression.strip()
    if s.startswith("function") or s.startswith("async "):
        return True
    first = s.split("\n", 1)[0]
    return "=>" in first


def _to_isolated_js(expression: str, arg: Any = _EVAL_ARG_UNSET) -> str | None:
    """Turn a Playwright evaluate() payload into a CDP isolated-world expression.

    Playwright's page.evaluate injects UtilityScript into the main world
    (rebrowser sourceUrlLeak). Isolated Runtime.evaluate does not.
    Returns None when *arg* cannot be JSON-serialized (ElementHandle, etc.).
    """
    arg_js: str | None
    if arg is _EVAL_ARG_UNSET:
        arg_js = None
    else:
        try:
            arg_js = json.dumps(arg)
        except (TypeError, ValueError):
            return None
    if _expression_is_function(expression):
        if arg_js is None:
            return f"({expression})()"
        return f"({expression})({arg_js})"
    if arg_js is not None:
        return None
    return expression


def _bind_isolated_evaluate(page: Any, orig_evaluate: Any, *, is_async: bool) -> None:
    """Replace page.evaluate with isolated-world CDP (no Playwright UtilityScript)."""

    if is_async:

        async def _stealth_evaluate(expression: str, arg: Any = _EVAL_ARG_UNSET) -> Any:
            world = getattr(page, "_stealth_world", None)
            js = _to_isolated_js(expression, arg)
            if isinstance(world, _AsyncIsolatedWorld) and js is not None:
                try:
                    return await world.evaluate(js)
                except Exception:
                    logger.debug("isolated evaluate failed; not using Playwright UtilityScript")
                    raise
            if arg is _EVAL_ARG_UNSET:
                return await orig_evaluate(expression)
            return await orig_evaluate(expression, arg)

        page.evaluate = _stealth_evaluate
        return

    def _stealth_evaluate(expression: str, arg: Any = _EVAL_ARG_UNSET) -> Any:
        world = getattr(page, "_stealth_world", None)
        js = _to_isolated_js(expression, arg)
        if isinstance(world, _SyncIsolatedWorld) and js is not None:
            try:
                return world.evaluate(js)
            except Exception:
                logger.debug("isolated evaluate failed; not using Playwright UtilityScript")
                raise
        if arg is _EVAL_ARG_UNSET:
            return orig_evaluate(expression)
        return orig_evaluate(expression, arg)

    page.evaluate = _stealth_evaluate


def ensure_stealth_evaluate(page: Any, *, is_async: bool = False) -> None:
    """Route page.evaluate through a CDP isolated world (rebrowser sourceUrlLeak).

    Idempotent. Used by humanize patch_page and by launch() so scrape-only
    sessions (humanize=False) still avoid Playwright UtilityScript.
    """
    if getattr(page, "_hexium_isolated_evaluate", None) is True:
        return
    world = getattr(page, "_stealth_world", None)
    if not isinstance(world, (_SyncIsolatedWorld, _AsyncIsolatedWorld)):
        try:
            page._stealth_world = (
                _AsyncIsolatedWorld(page) if is_async else _SyncIsolatedWorld(page)
            )
        except Exception:
            page._stealth_world = None
            logger.debug("Could not create isolated world for page.evaluate")
            return
    _bind_isolated_evaluate(page, page.evaluate, is_async=is_async)
    page._hexium_isolated_evaluate = True
...
def patch_page(page: Any, cfg: HumanConfig, cursor: _CursorState) -> None:
    ...
    from .cursor_overlay import attach_cursor_overlay_sync
    attach_cursor_overlay_sync(page, cfg, raw_mouse, cursor)
    ...
    def _human_click(selector: str, **kwargs: Any) -> None:
        ...
        human_move(raw_mouse, cursor.x, cursor.y, target.x, target.y, call_cfg, target_width=box["width"])
        cursor.x = target.x
        cursor.y = target.y
...
def patch_page_async(page: Any, cfg: HumanConfig, cursor: _CursorState) -> None:
    ...
    from .cursor_overlay import wrap_cursor_overlay_async
    wrap_cursor_overlay_async(page, cfg, raw_mouse, cursor)
    ...
    async def _human_click(selector: str, **kwargs: Any) -> None:
        ...
        await async_human_move(raw_mouse, cursor.x, cursor.y, target.x, target.y, call_cfg, target_width=box["width"])
        cursor.x = target.x
        cursor.y = target.y
