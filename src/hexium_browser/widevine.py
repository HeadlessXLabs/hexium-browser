...
Hexium's binary is built with Widevine support but ships no CDM (the CDM
...
def _seeding_disabled() -> bool:
    """True if HEXIUM_WIDEVINE is set to a falsey value (kill switch)."""
    val = os.environ.get("HEXIUM_WIDEVINE", "").strip().lower()
...
def resolve_widevine_cdm_dir(binary_path: str | os.PathLike) -> Path | None:
    ...
    Resolution order:
      1. If HEXIUM_WIDEVINE_CDM is set, it is used **exclusively** (overrides
      ...
      3. ``<cache dir>/WidevineCdm`` (``~/.hexium_browser/WidevineCdm``) — the
    ...
    """
    custom = os.environ.get("HEXIUM_WIDEVINE_CDM")
...
def seed_widevine_hint(user_data_dir: str | os.PathLike, binary_path: str | os.PathLike) -> None:
    ...
    HEXIUM_WIDEVINE, or when no sideloaded CDM is present. Never raises —
    a failure here must not break the browser launch.
    """
    if platform.system() != "Linux":
        return
    if _seeding_disabled():
        logger.debug("Widevine hint seeding disabled via HEXIUM_WIDEVINE")
        return
    ...
    try:
        ...
        if cdm_dir is None:
            if os.environ.get("HEXIUM_WIDEVINE_CDM") is not None:
                logger.warning(
                    "HEXIUM_WIDEVINE_CDM is set but has no manifest.json; "
