...
``~/.hexium_browser/geoip/``.  Background re-download after 30 days.
"""
...
DEFAULT_GEOIP_TIMEOUT_SECONDS = 30.0
GEOIP_TIMEOUT_ENV = "HEXIUM_GEOIP_TIMEOUT_SECONDS"
...
# Chrome --lang packs English as en-US / en-GB only. Region tags like en-PK
# are valid BCP 47 for navigator.languages / Accept-Language; --lang and LANG
# still use the pack Chromium actually ships (see chrome_lang_for_locale).
_CHROME_ENGLISH_LANG = {"en-US", "en-GB"}
_COMMONWEALTH_EN = {
    "GB", "AU", "NZ", "IE", "ZA", "SG", "PK", "NG", "KE", "GH", "UG", "ZM",
    "ZW", "JM", "TT", "MT", "IN", "HK", "MY", "BD",
}

# Country ISO code → BCP 47 locale (covers ~90 % of proxy traffic).
# English-speaking regions keep en-{CC} for navigator.languages; --lang is
# remapped separately via chrome_lang_for_locale.
COUNTRY_LOCALE_MAP: dict[str, str] = {
    ...
    "PK": "en-PK", "BD": "en-BD", "LK": "si-LK", "KZ": "ru-KZ", "IR": "fa-IR",
...
def normalize_chrome_locale(locale: str | None) -> str | None:
    """Map a BCP 47 tag to a value Chromium ``--lang`` actually honors.

    ``en-PK`` is a real locale; Chrome 151 has no language pack for it, so
    ``--lang=en-PK`` is ignored. Keep the region tag on navigator.languages
    and pass this pack to ``--lang`` / LANG instead.
    """
    if not locale:
        return locale
    if locale in _CHROME_ENGLISH_LANG:
        return locale
    lang, _, region = locale.partition("-")
    if lang == "en":
        if region in _COMMONWEALTH_EN:
            return "en-GB"
        return "en-US"
    return locale


def chrome_lang_for_locale(locale: str | None) -> str | None:
    """``--lang`` / LANG value that Chromium 151 will load."""
    return normalize_chrome_locale(locale)


def bcp47_to_posix_locale(locale: str) -> str:
    """``en-GB`` → ``en_GB.UTF-8`` so ICU cannot fall back to host LANG."""
    tagged = locale.replace("-", "_", 1)
    if "." not in tagged:
        tagged = f"{tagged}.UTF-8"
    return tagged
