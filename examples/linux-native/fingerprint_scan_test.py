...
Usage:
    python examples/linux-native/fingerprint_scan_test.py
    python examples/linux-native/fingerprint_scan_test.py --proxy http://10.50.96.5:8888
    python examples/linux-native/fingerprint_scan_test.py --headless
"""

import sys
import time

from hexium_browser import launch_context
...
def test_fingerprint_scan(page):
    ...
    shot = SCREENSHOTS_DIR / "fingerprint-scan.png"
...
def test_creepjs(page):
    ...
    shot = SCREENSHOTS_DIR / "creepjs.png"
    page.screenshot(path=str(shot), full_page=True)
...
def main():
    print("=" * 60)
    print("Hexium — Fingerprint & Headless Detection Tests")
    print("=" * 60)
    ...
    context = launch_context(
        headless=HEADLESS,
        humanize=True,
        show_cursor=False,  # overlay is a detector tell on stealth oracles
        proxy=PROXY,
        args=[
            "--fingerprint-screen-width=1920",
            "--fingerprint-screen-height=1080",
            "--hexium-timezone=Asia/Jerusalem",
        ],
    )
