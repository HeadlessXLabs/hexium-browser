"""Headed Allegro.pl — windows-chrome, HTTP proxy, ephemeral random profile.

Usage:
    export DISPLAY=:0
    export HEXIUM_BINARY_PATH="${HEXIUM_OUT}/chrome"
    export HEXIUM_PROXY='http://USER:PASS@host:port'
    python examples/windows-chrome/allegro_pl_headed_windows_chrome_http_proxy_ephemeral_random_profile.py
"""
...
def main() -> None:
    kw = allegro_launch_kwargs(headless=False, persona="windows-chrome")
    print(
        "Launching windows-chrome headed Allegro (proxy if HEXIUM_PROXY, ephemeral random profile)...",
        flush=True,
    )
    browser = launch(**kw)
    try:
